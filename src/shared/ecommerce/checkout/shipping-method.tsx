...
export default function ShippingMethod({ className }: { className?: string }) {
  ...
  return (
    <>
        ...
        <div className="rounded-lg border border-gray-200">
          <div className="p-4 @xs:p-6 @2xl:flex @2xl:items-start @2xl:justify-between @2xl:gap-6">
            <div className="block @5xl:pe-8">
              ...
              <Text as="p">Estimated delivery in 3-5 business days after order is shipped.</Text>
            </div>
              ...
              {shippingMethodData.map((item: any) => (
                <AdvancedRadio
                  ...
                  {/* <Image
                    ...
                    width={60}
                  /> */}
                  <img src={item.image} alt={item.name} height={45} width={60} />
                </AdvancedRadio>
              ))}
            </div>
          </div>
                ...
                {shippingSpeedData.map((item: any) => (
                  <AdvancedRadio
                    ...
                    <span className="block">
                      <Title as="h6" className="mb-1 block font-lexend text-sm font-medium">
                        {item.title}
                      </Title>
                      <Text as="span" className="block pe-8 font-normal leading-[1.85]">
                        {item.description}
                      </Text>
                    </span>
      ...
      {errors?.shippingMethod && <FieldError error={errors?.shippingMethod.message as string} />}
    </>
  );
}
