...
export const getColumns = ({ sortConfig, onDeleteItem, onHeaderCellClick, onChecked }: Columns) => [
      ...
      <div className="flex items-center">
        ...
        <div className="ms-3 pe-4">
          <Title as="h6" className="mb-1 !text-sm font-medium">
            <span className="font-normal text-gray-500">By</span> {customer.name}
          </Title>
  ...
  },
  {
    title: (
      <HeaderCell
        title="Rating"
        sortable
        ascending={sortConfig?.direction === 'asc' && sortConfig?.key === 'rating'}
      />
    ),
  ...
  },
  {
    ...
    key: 'product',
    width: 300,
    render: ({ name, category, image }: { name: string; category: string; image: string }) => (
      <AvatarCard
        src={image}
        name={name}
