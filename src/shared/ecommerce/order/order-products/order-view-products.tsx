...
const columns = [
    ...
    render: (_: any, row: CartItem) => (
      <div className="flex items-center">
        <div className="relative aspect-square w-12 overflow-hidden rounded-lg">
          ...
          /> */}
          <img alt={row.name} src={row.image} className="object-cover" />
        </div>
        <div className="ms-4">
  ...
  },
  {
    ...
    key: 'price',
    width: 200,
    render: (price: string) => <Text className="text-end text-sm">{toCurrency(price)}</Text>,
  },
  {
...

  {
    ...
    render: (price: number, row: CartItem) => (
      <Text className="text-end text-sm">{toCurrency(price * row.quantity)}</Text>
    ),
  },
];
