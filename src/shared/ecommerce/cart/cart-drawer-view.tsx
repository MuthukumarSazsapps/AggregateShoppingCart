...
export default function CartDrawerView({
  ...
  return (
    <div className="flex h-full w-full flex-col">
      <DrawerHeader heading="Shopping Cart" onClose={() => setOpenDrawer(false)} />
      {isCartEmpty ? (
      ...
      {isCartEmpty ? (
        <div className="px-4 py-5">
          <Button className="w-full" variant="flat" onClick={() => setOpenDrawer(false)}>
            Back To Shop
          </Button>
        </div>
      ) : (
        <Link
          ...
          className={cn(
            'mx-4 mb-6 mt-auto flex items-center justify-between rounded-md bg-gray-900 px-5 py-2 font-medium text-white dark:text-gray-0 md:mx-6',
          )}>
          Checkout
