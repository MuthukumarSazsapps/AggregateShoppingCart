...
export const getColumns = ({ sortConfig, onDeleteItem, onHeaderCellClick, onChecked }: Columns) => [
  {
    title: <></>,
    dataIndex: 'checked',
    key: 'checked',
  ...
  },
  {
    ...
    render: (image: any, row: any) => (
      <figure className="relative aspect-square w-12 overflow-hidden rounded-lg bg-gray-100">
        ...
        /> */}
        <img src={image} alt={row.name} className="object-cover" />
      </figure>
    ),
  },
  {
  ...
  },
  {
    ...
    key: 'description',
    width: 250,
    render: (description: string) => <Text className="truncate !text-sm ">{description}</Text>,
  },
  {
    title: (
      <HeaderCell
        title="Slug"
        sortable
        ascending={sortConfig?.direction === 'asc' && sortConfig?.key === 'slug'}
      />
    ),
  ...
  },
  {
    title: (
      <HeaderCell
        ...
        align="center"
        sortable
        ascending={sortConfig?.direction === 'asc' && sortConfig?.key === 'products'}
      />
    ),
  ...
  },
  {
    ...
    render: (_: string, row: any) => (
      <div className="flex items-center justify-end gap-3 pe-4">
        <Tooltip size="sm" content={() => 'Edit Category'} placement="top" color="invert">
          <Link to={routes.eCommerce.editCategory(row.id)}>
