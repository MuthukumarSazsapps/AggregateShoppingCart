...
import { ProductColor } from 'utils/types';
      ...
      <Controller
        ...
        render={({ field: { value } }) => (
          <div className="flex items-center gap-3">
              ...
              {colors.map(color => (
                <span
                  className={cn(
                    ...
                    value?.code === color.code && 'border-4 before:border before:border-gray-900',
                  )}
            ...
            {value && <span className="font-medium text-gray-500">{value?.name}</span>}
          </div>
        )}
      />
      {errors?.productColor && (
        <FieldError className="mt-1" error={errors?.productColor?.message as string} />
      )}
    </>
  );
}
