...
import { locationShipping } from 'shared/ecommerce/product/create-edit/form-utils';
...
export default function ShippingInfo({ className }: { className?: string }) {
  ...
  const addCustomField = useCallback(() => append([...locationShipping]), [append]);

  return (
    <FormGroup title="Shipping" description="Add your shipping info here" className={cn(className)}>
      <Controller
        name="freeShipping"
        control={control}
      ...
      {fields.map((item, index) => (
        <div key={item.id} className="col-span-full flex gap-4 xl:gap-7">
          ...
          />
          {fields.length > 1 && (
            <ActionIcon onClick={() => remove(index)} variant="flat" className="mt-7 shrink-0">
              <TrashIcon className="h-4 w-4" />
            </ActionIcon>
          )}
        </div>
      ))}
      <Button onClick={addCustomField} variant="outline" className="col-span-full ml-auto w-auto">
