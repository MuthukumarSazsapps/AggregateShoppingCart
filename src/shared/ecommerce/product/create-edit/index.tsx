...
import FormNav, { formParts } from 'shared/ecommerce/product/create-edit/form-nav';
import ProductSummary from 'shared/ecommerce/product/create-edit/product-summary';
import { defaultValues } from 'shared/ecommerce/product/create-edit/form-utils';
...
import DeliveryEvent from 'shared/ecommerce/product/create-edit/delivery-event';
import ProductVariants from 'shared/ecommerce/product/create-edit/product-variants';
...
export default function CreateEditProduct({ slug, product, className }: IndexProps) {
  ...
  const onSubmit: SubmitHandler<CreateProductInput> = data => {
    ...
    setTimeout(() => {
      ...
      toast.success(<Text as="b">Product successfully {slug ? 'updated' : 'created'}</Text>);
      methods.reset();
    }, 600);
  };
  ...
  return (
    <div className="@container">
      <FormNav className={cn(layout === LAYOUT_OPTIONS.BERYLLIUM && '2xl:top-[72px]')} />
