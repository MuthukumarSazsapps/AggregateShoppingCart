...
import { ProductDetailsInput, productDetailsSchema } from 'utils/validators/product-details.schema';
import { generateCartProduct } from 'store/quick-cart/generate-cart-product';

export default function ProductDetailsSummery({ product }: { product: Product }) {
      ...
      <FormProvider {...methods}>
        <form className="pb-8 pt-5" onSubmit={methods.handleSubmit(onSubmit)}>
          <div className="mb-1.5 mt-2 flex items-end font-lexend text-base">
            ...
            <div className="ps-1.5 text-red">({calculatePercentage(295, 320)}% OFF)</div>
          </div>
          <div className="font-medium text-green-dark">Inclusive of all taxes</div>
          ...
          <div className="grid grid-cols-1 gap-4 pt-7 @md:grid-cols-2 @xl:gap-6">
            <Button
              ...
              <PiShoppingCartSimple className="me-2 h-5 w-5 lg:h-[22px] lg:w-[22px]" /> Add To Cart
            </Button>
            <WishlistButton />
          </div>
        </form>
