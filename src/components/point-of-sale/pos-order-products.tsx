...
export default function POSOrderProducts({
  ...
  return (
    <div className={className}>
      <SimpleBar className={cn('h-[calc(100vh_-_630px)] pb-3', simpleBarClassName)}>
        <div className="space-y-7">
          {orderedItems.map(item => (
            <div
              key={item.id}
              className={cn('group relative flex items-center justify-between', itemClassName)}>
              <div className="flex w-4/6 items-start pe-2">
                ...
                <div className="ps-3">
                  <Title
                    ...
                    <Link to={routes.eCommerce.productDetails(generateSlug(item.name))}>
                      {item.name}
                    </Link>
                  </Title>
